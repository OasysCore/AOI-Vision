...
描述: 所有 OpenCV 算法在此包内，与业务层/UI层零耦合。
      可独立于 FastAPI 运行，通过 gRPC/Unix Socket 与业务层通信。
      发布时用 Cython 编译为 .so，保护核心算法。

架构层级:
  UI Layer (index.html)  ←HTTP→  Business Layer (FastAPI)  ←import→  Core Engine (aoi_engine)

依赖: opencv-python, numpy, pyzbar (optional)
"""
from aoi_engine.defect import DefectEngine
...
__all__ = ["DefectEngine", "VisionAnalyzer", "BaseCamera", "CameraProperties", "CalibrationEngine"]
__version__ = "0.2.0"
